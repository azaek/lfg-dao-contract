...
task('create-p-three', 'mint a profile and deploy governor and propose and vote').setAction(
    async ({}, hre) => {
        ...
        //Whitelisting all address

        for (const addres of accounts) {
            await waitForTx(lensHub.whitelistProfileCreator(addres.address, true));
        }

        console.log(`whitelisted every address`);
        ...
        // await waitForTx(lensHub.connect(member).createProfile(inputStruct));

        // const profileId = await lensHub.getProfileIdByHandle(username);
        // console.log(`\n\t --- profile minted : ${profileId}`);
